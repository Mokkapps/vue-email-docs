...
<template>
  <div ref="target" class="flex flex-col transition-opacity duration-500" :class="targetIsVisible ? 'opacity-100' : 'opacity-25'">
    ...
    <NuxtLink v-if="date.release" :to="`https://github.com/vue-email/vue-email/releases/tag/${date.release.name}`" target="_blank" class="text-gray-900 dark:text-white font-bold text-3xl mt-2 group hover:text-primary-500 dark:hover:text-primary-400 transition-[color]">
      {{ date.release.name }}
    </NuxtLink>
